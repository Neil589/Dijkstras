...
public interface Graph {
  ...
  /**
   * @param label - string for name of a5.Node, must be unique (cannot be the same as an existing
   *     node in the graph)
   * @return false if label is not unique (or is null) true if node is successfully added
   */
   ...
   * @return false if edge weight is less than 0 false if source node is not in graph false if
   *     destination node is not in graph false is there already is an edge between these 2 nodes
   ...
   * @return false if the node does not exist true if the node is found and successfully removed
   */
   ...
   * @return false if the edge does not exist true if the edge is found and successfully removed
   */
   ...
   * Return a map where the key is the name of the node and the value is the distance from the start
   * node. The start node should be included in the returned map, with the value 0 as the distance.
   *
